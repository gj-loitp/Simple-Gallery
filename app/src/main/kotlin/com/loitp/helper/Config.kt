...
class Config(context: Context) : BaseConfig(context) {
    ...
    fun saveFolderGrouping(path: String, value: Int) {
        if (path.isEmpty()) {
            ...
            prefs.edit().putInt(GROUP_FOLDER_PREFIX + path.toLowerCase(Locale.getDefault()), value)
                .apply()
        }
    }
    ...
    fun getFolderGrouping(path: String): Int {
        var groupBy =
            prefs.getInt(GROUP_FOLDER_PREFIX + path.toLowerCase(Locale.getDefault()), groupBy)
    ...
    fun removeFolderGrouping(path: String) {
        prefs.edit().remove(GROUP_FOLDER_PREFIX + path.toLowerCase(Locale.getDefault())).apply()
    }

    fun hasCustomGrouping(path: String) =
        prefs.contains(GROUP_FOLDER_PREFIX + path.toLowerCase(Locale.getDefault()))

    fun saveFolderViewType(path: String, value: Int) {
        if (path.isEmpty()) {
            viewTypeFiles = value
        } else {
            prefs.edit().putInt(VIEW_TYPE_PREFIX + path.toLowerCase(Locale.getDefault()), value)
                .apply()
        }
    }

    fun getFolderViewType(path: String) =
        prefs.getInt(VIEW_TYPE_PREFIX + path.toLowerCase(Locale.getDefault()), viewTypeFiles)

    fun removeFolderViewType(path: String) {
        prefs.edit().remove(VIEW_TYPE_PREFIX + path.toLowerCase(Locale.getDefault())).apply()
    }

    fun hasCustomViewType(path: String) =
        prefs.contains(VIEW_TYPE_PREFIX + path.toLowerCase(Locale.getDefault()))
    ...
    fun addExcludedFolder(path: String) {
        addExcludedFolders(HashSet(mutableListOf(path)))
    }
    ...
    private fun getDefaultDirectoryColumnCount() = context.resources.getInteger(
        if (scrollHorizontally) R.integer.directory_columns_horizontal_scroll
    ...
    private fun getDefaultMediaColumnCount() = context.resources.getInteger(
        if (scrollHorizontally) R.integer.media_columns_horizontal_scroll
    ...
    fun removeLastVideoPosition(path: String) {
        prefs.edit().remove("$LAST_VIDEO_POSITION_PREFIX${path.toLowerCase(Locale.getDefault())}")
            .apply()
    }
    ...
    fun saveLastVideoPosition(path: String, value: Int) {
        if (path.isNotEmpty()) {
            prefs.edit().putInt(
                "$LAST_VIDEO_POSITION_PREFIX${path.toLowerCase(Locale.getDefault())}",
                value
            ).apply()
        }
    }

    fun getLastVideoPosition(path: String) =
        prefs.getInt("$LAST_VIDEO_POSITION_PREFIX${path.toLowerCase(Locale.getDefault())}", 0)
